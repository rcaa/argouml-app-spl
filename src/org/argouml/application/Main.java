...
import org.argouml.notation.InitNotation; //JAVA
 ...
 * invocation: non-GUI command line and Swing GUI.
 * <p>
 * 
 * NOTE: Functionality which should be common to all types of application
 * invocation (e.g. extension modules to be loaded) should added to some common
 * class and <b>not</b> here. Adding things here will cause behavior to diverge
 * for other application invocation types (e.g. ArgoEclipse).
 * 
 */
public class Main {
    ...
    public static void main(String[] args) {
        try {
            ...
            URL urlToOpen = null;
            if (projectName != null) {
                projectName = PersistenceManager.getInstance().fixExtension(
                        projectName);
    ...
    private static void initTranslator() {
        ...
        org.argouml.cognitive.Translator
                .setTranslator(new AbstractCognitiveTranslator() {
    ...
    private static void parseCommandLine(String[] args) {
        ...
        reloadRecent = Configuration.getBoolean(Argo.KEY_RELOAD_RECENT_PROJECT,
                false);
        ...
        for (int i = 0; i < args.length; i++) {
            if (args[i].startsWith("-")) {
                ...
                if (themeName != null) {
                    ...
                    String projectToBePrinted = PersistenceManager
                            .getInstance().fixExtension(args[++i]);
    ...
    // JAVA
    private static void initNotationJavaHook() {
        // SubsystemUtility.initSubsystem(new InitNotationJava());
    }

    // UML
    private static void initNotationUmlHook() {
            ...
            if (splash != null) {
                ...
                splash.getStatusBar().showStatus(
                        Translator.messageFormat(
                                "statusmsg.bar.readingproject", msgArgs));
            }
    ...
    private static String getMostRecentProject() {
        ...
        if (!("".equals(s))) {
            ...
            if (file.exists()) {
                ...
                LOG.warn("Cannot re-open " + s + " because it does not exist");
            }
        }
        return null;
    }
    ...
    /**
     * Calculates the {@link URL} for the given project name. If the file does
     * not exist or cannot be converted the default {@link URL} is returned.
     * 
    ...
    private static void printUsage() {
        ...
        /*
         * TODO: The Quickguide also mentions: -open <arg> open given file on
         * startup -print <arg> print given file on startup (and exit) Why are
         * these gone?
         */
        System.err.println("");
    ...
    /**
     * Check that we can get the InetAddress for localhost. This can fail on
     * Unix if /etc/hosts is not correctly set up.
     */
    ...
    /**
     * Add an element to the PostLoadActions list, which contains actions that
     * are run after ArgoUML has started.
     * 
     ...
     * 
     * This first implementation just has a list of commands that is possible to
     * give.
     * 
    ...
    /**
     * Create the .argouml directory if it doesn't exist. This is done here
     * because it must be done before setting the log configuration.
     */
    static {
    ...
    /**
     * Install our security handlers, and do basic initialization of log4j.
     * 
     * Log4j initialization must be done as part of the main class initializer,
     * so that the log4j initialization is complete before any other static
     * initializers.
     * 
     * Also installs a trap to "eat" certain SecurityExceptions. Refer to
     * {@link java.awt.EventDispatchThread} for details.
     */
    static {
        ...
        /*
         * The string <code>log4j.configuration</code> is the same string found
         * in {@link org.apache.log4j.Configuration.DEFAULT_CONFIGURATION_FILE}
         * but if we use the reference, then log4j configures itself and clears
         * the system property and we never know if it was set.
         * 
         * If it is set, then we let the static initializer in {@link Argo}
    ...
    private static ProjectBrowser initializeGUI(SplashScreen splash) {
        ...
        int configFrameHeight = Configuration.getInteger(
                Argo.KEY_SCREEN_HEIGHT, scrSize.height);
        ...
        pb.setLocation(x, y);
        pb.setSize(w, h);
        pb.setExtendedState(Configuration.getBoolean(Argo.KEY_SCREEN_MAXIMIZED,
     ...
     * <p>
     * 
     * This function is intentionally public, since applications built on
     * ArgoUML, that do not make use of Main.main(), can call this function and
     * then access ArgoUML's version from the ApplicationVersion class.
     */
...
/**
 * Class to hold a list of actions to be perform and to perform them after the
 * initializations is done.
 */
...
class LoadModules implements Runnable {
    ...
    /**
     * Load internal modules which should be found on the standard classpath.
     */
